module ApplicationHelper
	...
	def s (setting)
		if !params[setting].nil?&&app_set('use_params')
		  out=params[setting].value.to_s.html_safe
		else
		  out = Setting.find_by_key(setting)  
		  !(out.nil?) ? out=out.value.to_s.html_safe : out= app_set("out_nils") ? nil : "<missing_setting_#{setting}>".html_safe
		  params[setting]=out
		  logger.debug("setting #{setting} = #{out}")
		end
		return out
    end
	...
	def app_set(key)
		if set=Setting.find_by_key(key) then
		   case Setting.find_by_key(key).keyclass
		   when 'String'
			   return set.value
		   when 'Integer'
